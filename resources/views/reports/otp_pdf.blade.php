...
<html>
<head>
    <title>Lembar OTP Verifikasi Akun - {{ $personel->full_name }}</title>
    <style>
        ...
        .otp-code { font-size: 16px; font-weight: bold; letter-spacing: 4px; text-align: center; }
...
<body>
    ...
    <div class="title-block">
        <div class="title">LEMBAR KODE OTP VERIFIKASI AKUN</div>
        <div class="subtitle">NOMOR: {{ $nomorSurat }}</div>
        <div class="info-row">Diterbitkan: {{ $generatedAt }} &nbsp;|&nbsp; Berlaku hingga: {{ $expiredAt }}</div>
    </div>

    {{-- TABEL DATA PERSONEL + OTP --}}
    <table>
        <thead>
            <tr>
                <th style="width: 4%;">NO</th>
                <th style="width: 18%;">NIKC</th>
                <th>NAMA LENGKAP</th>
                <th style="width: 11%;">PANGKAT</th>
                <th style="width: 8%;">MATRA</th>
                <th style="width: 14%;">NOMOR HP</th>
                <th style="width: 13%;">KODE OTP</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td class="text-center">1</td>
                <td class="text-center">{{ $personel->nikc ?? '-' }}</td>
                <td>{{ $personel->full_name }}</td>
                <td class="text-center">{{ $personel->pangkat ?? '-' }}</td>
                <td class="text-center">{{ $personel->matra }}</td>
                <td>{{ $personel->phone_number }}</td>
                <td class="otp-code">{{ $personel->manual_otp }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="instruction-box">
        <strong>PETUNJUK PENGGUNAAN OTP:</strong>
        <ol>
            <li>Buka halaman <strong>sisfoperskc.my.id</strong> dan login menggunakan NIKC Anda.</li>
            <li>Setelah login, sistem akan menampilkan halaman <strong>Verifikasi OTP</strong>.</li>
            <li>Pilih opsi <strong>"Gunakan Kode OTP Manual"</strong> di bagian bawah halaman verifikasi.</li>
            <li>Masukkan 6 digit Kode OTP yang tertera pada tabel di atas secara tepat.</li>
            <li>Klik tombol <strong>Verifikasi</strong>. Akun Anda akan langsung aktif.</li>
            <li><em>OTP ini bersifat RAHASIA — jangan berikan kepada pihak lain selain personel bersangkutan.</em></li>
        </ol>
    </div>
