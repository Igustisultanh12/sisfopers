  ...
  <AuthenticatedLayout>
    <div class="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 class="text-xl font-extrabold text-slate-800 tracking-tight">Generate OTP Reset Password</h1>
          <p class="text-xs text-slate-400 mt-1">Kelola kode OTP Reset Password untuk personel aktif yang ingin memperbarui kata sandi.</p>
        </div>
        <div class="flex flex-wrap gap-2">
          ...
          >
            ...
            Generate Semua OTP Reset
          </button>
          <!-- Tombol Cetak Semua PDF -->
          <a
            :href="route('admin.otp.reset-password.bulk-pdf')"
            target="_blank"
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div class="bg-white border border-[#E2E8F0] rounded-2xl p-4 text-center shadow-xs">
          <p class="text-2xl font-extrabold text-slate-700">{{ personels.total }}</p>
          <p class="text-xs text-slate-400 mt-1 font-medium">Total Personel Aktif</p>
        </div>
        <div class="bg-white border border-[#E2E8F0] rounded-2xl p-4 text-center shadow-xs">
          <p class="text-2xl font-extrabold text-amber-600">{{ countHasOtp }}</p>
          <p class="text-xs text-slate-400 mt-1 font-medium">Sudah Punya OTP Reset</p>
        </div>
        <div class="bg-white border border-[#E2E8F0] rounded-2xl p-4 text-center shadow-xs">
          <p class="text-2xl font-extrabold text-slate-400">{{ countNoOtp }}</p>
          <p class="text-xs text-slate-400 mt-1 font-medium">Belum Generate OTP Reset</p>
        </div>
      </div>
          ...
          <table class="w-full text-xs">
            <thead class="bg-slate-50 border-b border-[#E2E8F0]">
              <tr>
                ...
                <th class="text-center px-5 py-3.5 font-bold text-slate-500 uppercase tracking-wider">Status OTP Reset</th>
            ...
            <tbody class="divide-y divide-[#F1F5F9]">
              <tr v-for="p in personels.data" :key="p.id" class="hover:bg-slate-50/60 transition">
                ...
                <td class="px-5 py-4 text-center">
                  <!-- Sudah Dicetak -->
                  <span
                    v-if="p.reset_password_otp && p.reset_password_otp_printed_at"
                  ...
                  <span
                    v-else-if="p.reset_password_otp && !p.reset_password_otp_printed_at"
                ...
                <td class="px-5 py-4">
                  <div class="flex items-center justify-center gap-1.5 flex-wrap">
                    ...
                    >
                      ...
                      {{ p.reset_password_otp ? 'Regen' : 'Generate' }}
                    </button>

                    <!-- Tombol Cetak PDF (hanya jika sudah ada OTP) -->
                    <a
                      v-if="p.reset_password_otp"
                      :href="route('admin.otp.reset-password.pdf', p.id)"
                      target="_blank"
              ...
              <tr v-if="personels.data.length === 0">
                <td colspan="6" class="px-5 py-14 text-center text-slate-400 text-xs font-medium">
                  ...
                  Tidak ada data personel yang cocok dengan kriteria pencarian.
                </td>
              </tr>
            </tbody>
          </table>
...
const applyFilter = () => {
  router.get(route('admin.otp.reset-password.index'), {
    search: searchQuery.value,
    matra:  filterMatra.value,
  }, { preserveState: true, replace: true });
};

const countHasOtp = computed(() => props.personels.data.filter(p => p.reset_password_otp).length);
const countNoOtp  = computed(() => props.personels.data.filter(p => !p.reset_password_otp).length);

// Generate OTP untuk satu personel
const confirmGenerate = async (personel) => {
  const isRegen = !!personel.reset_password_otp;
  const confirmed = await confirmAction(
    isRegen ? 'Regenerate OTP Reset?' : 'Generate OTP Reset Password?',
    isRegen
      ? `OTP Reset Password lama untuk ${personel.full_name} akan diganti yang baru. Lembar cetak lama tidak berlaku lagi.`
      : `Buat kode OTP Reset Password 6 digit untuk ${personel.full_name}? OTP berlaku 24 jam dan dapat dicetak ke PDF.`,
    isRegen ? 'Ya, Regenerate!' : 'Ya, Generate!'
  );

  if (confirmed) {
    showLoadingProgress('Sedang membuat kode OTP Reset Password...');
    router.post(route('admin.otp.reset-password.generate', personel.id), {}, {
      onSuccess: () => {
        closeLoading();
        alertSuccess('Berhasil', 'Kode OTP Reset Password berhasil dibuat.');
      },
      onError: () => {
        closeLoading();
...
// Generate OTP untuk SEMUA personel
const confirmGenerateAll = async () => {
  const confirmed = await confirmAction(
    'Generate Semua OTP Reset?',
    `Sistem akan membuat kode OTP Reset Password baru untuk semua personel aktif. Kode lama akan di-regenerate.`,
    'Ya, Generate Semua!'
  );

  if (confirmed) {
    showLoadingProgress('Sedang generate OTP Reset untuk semua personel...');
    router.post(route('admin.otp.reset-password.generate-all'), {}, {
      onSuccess: () => {
        closeLoading();
        alertSuccess('Berhasil', 'Semua kode OTP Reset Password berhasil dibuat.');
      },
      onError: () => {
        closeLoading();
